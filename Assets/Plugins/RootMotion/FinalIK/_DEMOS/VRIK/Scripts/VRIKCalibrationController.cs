...
namespace RootMotion.Demos
{
    ...
    public class VRIKCalibrationController : MonoBehaviour
    {
        ...
        [ContextMenu("0_Callibrate_Data")]
        public void Calibrate_Data()
        {
            Debug.Log("Callibrating Avatar");
            data = VRIKCalibrator.Calibrate(ik, settings, headTracker, bodyTracker, leftHandTracker, rightHandTracker, leftFootTracker, rightFootTracker);
        }

        [ContextMenu("0.1_Recallibrate")]
        public void Recallibrate()
        {
            Debug.Log("Recallibrating Avatar");
            if (data.scale == 0f)
            {
                Debug.LogError("No Calibration Data to calibrate to, please calibrate with settings first.");
            }
        ...
        [ContextMenu("1_Rescale_Avatar")]
        public void Rescale_Avatar()
        {
            Debug.Log("Rescaling Avatar");
            if (data.scale == 0f)
            {
                Debug.LogError("Avatar needs to be calibrated before RecalibrateScale is called.");
            }
        ...
        void Start_Callibration_Process()
        {
            Debug.Log("Starting coroutine");
            enumerator = Callibrate_I();
            StartCoroutine(enumerator);
        }

        IEnumerator Callibrate_I()
        {
            yield return new WaitForSeconds(1);
            Debug.Log("1. Callibrating...");
            Calibrate_Data();
            yield return new WaitForSeconds(1);
            Debug.Log("1. Scaling...");
            Rescale_Avatar();
